...
def detect_spams(detector, train, test, delta=1, freq_threshold=5):
    ...
    sol = imp.load_source(name, detector)
    ...
    erreur_train = mean([y != sol.predire(x, P, TARGETS, delta=delta)[0] for x, y in train_corpus])
    print ('Erreur de classification sur ensemble d\'entraînement (' + str(len(train_corpus)) + ' items): %.4f' % (erreur_train * 100) + '%')

    erreur_test = mean([y != sol.predire(x, P, TARGETS, delta=delta)[0] for x, y in test_corpus])
    print ('Erreur de classification sur ensemble de test (' + str(len(test_corpus)) + ' items): %.4f' % (erreur_test * 100) + '%')
...
def validate_detector(detector, validation_file, delta=1):
    ...
    with open(validation_file, 'rb') as f:
        res_train, res_train_corpus, res_V, res_prob, res_nbMotsParClasse, res_nbDocsParClasse, res_freqWC = pickle.load(f, encoding='latin1')
    ...
    same_words = all([w1 == w2 for w1, w2 in zip(res_train_corpus[0][0], testing_pretraiter)])
    ...
    same_nbMotsParClasse = compare_dict(testing_P.nbMotsParClasse, res_nbMotsParClasse)
    same_nbDocsParClasse = compare_dict(testing_P.nbDocsParClasse, res_nbDocsParClasse)
    ...
    testing_prob = sol.predire(res_train_corpus[0][0], res_P, TARGETS, delta=delta)
...
def main():
    ...
    if not os.path.isfile(detector):
        parser.error("-detecteur '{0}' doit être un fichier!".format(os.path.abspath(detector)))

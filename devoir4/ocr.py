...
def show_recognized_characters(reseau_de_neurones, dataset):
    ...
    for x, y in zip(*dataset):
        ...
        plt.imshow(x.reshape(16, 8), cmap=plt.cm.Greys, interpolation='nearest')
        plt.xticks([])
        plt.yticks([])
...
def recognize_characters(ocr, train, test, validation_file, T=25, alpha=0.1, n_neurones_caches=10):
    ocr = os.path.abspath(ocr)
    name = ocr.replace('/', '.').replace('.', '_')
    sol = imp.load_source(name, ocr)
    ...
    erreur_train = np.mean([y != reseau_de_neurones.prediction(x) for x, y in zip(X, Y)])
    print ('Erreur de classification sur ensemble d\'entraînement:', str(erreur_train * 100) + '%')

    erreur_test = np.mean([y != reseau_de_neurones.prediction(x) for x, y in zip(X_test, Y_test)])
...
def validate_ocr(reseau_de_neurones, validation_file):
    ...
    diff_w = ((w_attendu - w) ** 2).sum()
    if diff_W < 1e-20:
        print ('Différence entre le W trouvé et celui attendu:', diff_W, '(RÉUSSI)')
    else:
